...
**Symptom:** `dbq practice-mgmt 'db.getCollectionNames()'` returns an empty list with `ok: 1` and no
...
named `practice-mgmt` exists, so `listCollections` legitimately succeeded against an empty namespace.

**Handling:** name the database explicitly. `dbq --collections practice-mgmt` with no database selected
lists the databases rather than silently returning nothing.

    DBQ_DB=<database> dbq --collections practice-mgmt
    dbq practice-mgmt 'db.getSiblingDB("<database>")["<collection>"].countDocuments({})'
