...
**Symptom:** `dbq tenant-mgmt 'db.getCollectionNames()'` returns an empty list with `ok: 1` and no
...
named `tenant-mgmt` exists, so `listCollections` legitimately succeeded against an empty namespace.

**Handling:** name the database explicitly. `dbq --collections tenant-mgmt` with no database selected
lists the databases rather than silently returning nothing.

    DBQ_DB=<database> dbq --collections tenant-mgmt
    dbq tenant-mgmt 'db.getSiblingDB("<database>")["<collection>"].countDocuments({})'
