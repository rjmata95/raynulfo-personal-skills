...
**Symptom:** `dbq patient 'db.getCollectionNames()'` returns an empty list with `ok: 1` and no
...
named `patient` exists, so `listCollections` legitimately succeeded against an empty namespace.

**Handling:** name the database explicitly. `dbq --collections patient` with no database selected
lists the databases rather than silently returning nothing.

    DBQ_DB=<database> dbq --collections patient
    dbq patient 'db.getSiblingDB("<database>")["<collection>"].countDocuments({})'
